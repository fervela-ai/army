...
export function scoreMove(game, seat, memory, { from, to }) {
  ...
  if (piece === '炸彈') {
    ...
    // 炸彈走進**任何**行營都算數（Lynch 2026-09-02 的算式，用他的代號）：
    //   「L32 走到 L22 就可以保護 L11、L12、L13、L21、L31、L33。」
    //   行營裡誰都吃不到它，而它一步就能打到周圍六格——比貼在大子旁邊還好，
    //   因為對手看不出那顆是什麼，也不敢隨便碰旁邊的子。
    // 原本只認自家門前那兩個（r4c2／r4c4），等於把這招限縮成守家一種用法。
    // ⚠ 但不能每局都變成「行營裡就是炸彈」——Lynch 特別叮嚀過。
    //   所以這是判斷類權重（會隨性格抖動），不是鐵律；而且開局不做
    //   （開局的行營要拿來佔位，藏炸彈是中盤的事）。
    else if (!opening && kindOf(to) === 'camp' && kindOf(from) !== 'camp')
