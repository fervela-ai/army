...
import { SEATS } from '../engine/src/board.mjs?v=220';
import { legalMoves as calcLegalMoves, validateSetup, movePath } from '../engine/src/rules.mjs?v=220';
import { GAME_WS } from './config.js?v=220';
import { ensureAccount } from './account.js?v=220';
import { gameStats } from '../engine/src/game-stats.mjs?v=220';
...
export async function remoteSession({ code, nickname, onState, onError } = {}) {
  ...
  return {
    ...
    // ⚠ 還要標出「相對於我」的方位：三家電腦全叫「電腦」的話，戰報上三行
    //    「電腦：移動」根本分不出是誰（GPT 實測回饋，Lynch 同意要修）。
    //    方位是相對的——行棋順序逆時針，我的下一家是右家、對面是對家（隊友）、再來是左家。
    seatNames: () => {
      const me = mySeats()[0] ?? 0;
      const where = (s) => ['', '右家', '對家・隊友', '左家'][(s - me + 4) % 4];
      return SEATS.map(s => {
        const info = state?.seats?.[s];
        if (!info) return `P${s}`;
        if (mySeats().includes(s)) return `${info.nickname}（你）`;
        const tag = where(s);
        const off = !info.ai && !info.connected ? '・離線' : '';
        return `${info.nickname}（${tag}${off}）`;
      });
    },
